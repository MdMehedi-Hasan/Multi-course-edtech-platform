  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 text-white rounded-3xl p-8 border border-slate-800 shadow-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <p className="text-xs sm:text-sm text-slate-300 mt-1 max-w-xl">
            You have completed <strong className="text-white">{completedCoursesCount}</strong>{' '}
            courses so far. Keep up the streak and advance your engineering skillset!
          </p>
        </div>
        ...
        <Card className="p-5 hover:shadow-md transition-shadow">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h3 className="text-2xl font-extrabold text-slate-100">{totalCourses}</h3>
            </div>
          </div>
        </Card>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h3 className="text-2xl font-extrabold text-slate-100">
                {totalCourses - completedCoursesCount}
              </h3>
            </div>
          </div>
        </Card>
        ...
        <Card className="p-5 hover:shadow-md transition-shadow">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h3 className="text-2xl font-extrabold text-slate-100">{overallProgress}%</h3>
            </div>
          </div>
        </Card>
      </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-bold text-slate-100 flex items-center gap-2">
              <Clock className="w-5 h-5 text-indigo-400" /> Continue Learning
            </h2>
          </div>
          <Card className="p-6 bg-gradient-to-r from-indigo-500/10 via-slate-800 to-slate-800 relative overflow-hidden">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
              <div className="space-y-3 max-w-2xl">
                <div className="flex items-center gap-2">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-indigo-300 bg-indigo-500/15 px-2.5 py-0.5 rounded">
                    Active Course
                  </span>
                ...
                <div className="flex items-center gap-3 text-xs text-slate-400">
                  <span className="font-semibold text-indigo-400">Next Lesson:</span>
                ...
                <Button
                  ...
                  onClick={() =>
                    continueLearning.nextLessonId
                      ? onNavigate(
                          `/student/learn/${continueLearning.courseId}/${continueLearning.nextLessonId}`
                        )
                      : onNavigate(`/student/courses/${continueLearning.courseId}`)
                  }
                  className="gap-2.5 w-full lg:w-auto"
                >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {enrolledCourses.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {enrolledCourses.slice(0, 4).map((enr: any) => (
                <Card
                  key={enr.id}
                  className="p-5 flex flex-col justify-between hover:border-slate-600 transition-colors"
                >
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-xs font-bold text-indigo-400">{enr.progress}%</span>
                    </div>
                    <h4 className="font-bold text-slate-100 text-sm mb-1 line-clamp-1">
                      {enr.title}
                    </h4>
                    <p className="text-xs text-slate-400 mb-4">By {enr.instructorName}</p>
        ...
        <div className="space-y-4">
          <h2 className="text-lg font-bold text-slate-100 flex items-center gap-2">
            <Award className="w-5 h-5 text-amber-400" /> Earned Certificates
          </h2>
            ...
            {recommendedCourses.map((rc: any) => (
              ...
              >
                <div>
                  ...
                  <h4 className="font-bold text-slate-100 text-sm mt-2 mb-1">{rc.title}</h4>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-slate-700/50">
                  <span className="font-extrabold text-slate-100 text-sm">${rc.price}</span>
                  <Button
                    variant="default"
                    size="sm"
      ...
      {selectedCert && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-800 rounded-3xl max-w-2xl w-full p-8 border border-slate-700 shadow-2xl relative">
            <div className="border-4 border-amber-400 p-8 rounded-2xl text-center bg-gradient-to-b from-amber-500/10 to-slate-800 relative">
              <span className="text-xs font-bold uppercase tracking-widest text-amber-300 bg-amber-500/15 px-3 py-1 rounded-full">
                Certificate of Completion
              </span>
              <p className="text-xs text-slate-400 mt-4 uppercase tracking-wider">
                This is to certify that
              </p>
              <h2 className="text-2xl font-black text-white mt-1">{selectedCert.studentName}</h2>
              ...
              <p className="text-xs text-slate-400 mt-4">
                Instructor: {selectedCert.instructorName}
              </p>
