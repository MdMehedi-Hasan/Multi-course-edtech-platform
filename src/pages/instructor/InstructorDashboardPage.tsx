    ...
    <div className="space-y-8">
        ...
        <Card className="p-4 border-slate-700/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h3 className="text-xl font-black text-slate-100">{totalCourses}</h3>
            </div>
          </div>
        </Card>
        ...
        <Card className="p-4 border-slate-700/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h3 className="text-xl font-black text-slate-100">{publishedCourses}</h3>
            </div>
          </div>
        </Card>
        ...
        <Card className="p-4 border-slate-700/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h3 className="text-xl font-black text-slate-100">{draftCourses}</h3>
            </div>
          </div>
        </Card>
        ...
        <Card className="p-4 border-slate-700/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h3 className="text-xl font-black text-slate-100">{totalStudents}</h3>
            </div>
          </div>
        </Card>
        ...
        <Card className="p-4 border-slate-700/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h3 className="text-xl font-black text-slate-100">${totalRevenue.toFixed(2)}</h3>
            </div>
          </div>
        </Card>
        ...
        <Card className="p-4 border-slate-700/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h3 className="text-xl font-black text-slate-100">{averageRating}</h3>
            </div>
          </div>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-bold text-slate-100 flex items-center gap-2">
              <TrendingUp className="w-5 h-5 text-indigo-400" /> Course Performance & Revenue
            </h2>
            <Button
              variant="outline"
              size="sm"
          ...
          <div className="bg-slate-900/60 rounded-2xl border border-slate-700/50 overflow-hidden shadow-xs">
              ...
              <tbody className="divide-y divide-slate-700/50">
                {coursePerformance.length === 0 ? (
                  ...
                  coursePerformance.map((cp: any) => (
                    <tr key={cp.id} className="hover:bg-slate-700/40 transition-colors">
                      <td className="p-4 font-bold text-slate-100 line-clamp-1 max-w-xs">{cp.title}</td>
                      <td className="p-4">
                        {cp.isPublished ? (
                      ...
                      <td className="p-4 font-bold text-slate-200">{cp.studentCount}</td>
                      <td className="p-4 text-slate-400">${cp.price.toFixed(2)}</td>
                      <td className="p-4 font-black text-slate-100">${cp.revenue.toFixed(2)}</td>
        ...
        <div className="lg:col-span-1 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-bold text-slate-100">Recent Enrollments</h2>
            <Button
              variant="ghost"
              size="sm"
              onClick={() => onNavigate('/instructor/students')}
              className="text-xs text-indigo-400 hover:text-indigo-400"
            >
              View All
            </Button>
          </div>

          <Card className="p-4 border-slate-700/50 space-y-3 divide-y divide-slate-700/50">
            {recentEnrollments.length === 0 ? (
              <p className="p-4 text-center text-xs text-slate-400">No student enrollments logged yet.</p>
            ) : (
              recentEnrollments.map((e: any) => (
                <div key={e.id} className="pt-3 first:pt-0 flex items-center justify-between gap-3 text-xs">
                  <div className="min-w-0">
                    <p className="font-bold text-slate-100 truncate">{e.studentName}</p>
                    <p className="text-[11px] text-slate-400 truncate">{e.courseTitle}</p>
                    <p className="text-[10px] text-slate-400 mt-0.5">
                      {new Date(e.enrolledAt).toLocaleDateString()}
                    </p>
                  </div>
                  <span className="text-xs font-black text-emerald-400 bg-emerald-500/10 px-2 py-1 rounded shrink-0">
                    +${e.coursePrice}
                  </span>
                </div>
              ))
