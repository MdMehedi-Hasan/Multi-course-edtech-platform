...
export const AdminEnrollmentsPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-extrabold text-slate-100 tracking-tight">Global Enrollment Ledger</h1>
        <p className="text-xs text-slate-400 mt-1">Audit student registrations, track progress across all courses, and inspect completion status.</p>
      </div>
            ...
            <tbody className="divide-y divide-slate-700/50">
              {isLoading ? (
                ...
                filteredEnrollments.map((enr) => (
                  <tr key={enr.id} className="hover:bg-slate-700/40 transition-colors">
                    <td className="px-5 py-4">
                      <p className="font-bold text-slate-100">{enr.studentName}</p>
                      <p className="text-[11px] text-slate-400">{enr.studentEmail}</p>
                    </td>

                    <td className="px-4 py-4">
                      <p className="font-bold text-purple-400 line-clamp-1">{enr.courseTitle}</p>
                    ...
                    <td className="px-4 py-4 w-48">
                      <div className="flex items-center gap-2 mb-1">
                        <div className="flex-1 bg-slate-700/60 rounded-full h-2 overflow-hidden border border-slate-700/50">
                          <div
                            className="bg-purple-600 h-full transition-all"
                            style={{ width: `${enr.progress}%` }}
                          />
                        </div>
                        <span className="font-extrabold text-slate-200 text-[11px]">{enr.progress}%</span>
                      </div>
                      {enr.completedAt && (
