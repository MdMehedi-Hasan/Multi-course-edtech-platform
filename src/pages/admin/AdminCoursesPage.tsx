...
export const AdminCoursesPage: React.FC<AdminCoursesPageProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight">Course Inventory & Quality Governance</h1>
        <p className="text-xs text-slate-500 mt-1">Approve curriculum quality, manage global availability, highlight featured courses, and enforce content policies.</p>
      </div>
              ...
              {isLoading ? (
                ...
                courses.map((c) => (
                  <tr key={c.id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <div className="flex items-center gap-3">
                        <div className="w-12 h-10 rounded-lg bg-slate-800 shrink-0 overflow-hidden border border-slate-700">
                          {c.thumbnailUrl ? (
                            ...
                            <div className="w-full h-full flex items-center justify-center text-slate-500 font-bold text-[10px]">COURSE</div>
                          )}
                        </div>
                        <div>
                    ...
                    <td className="px-4 py-4 font-extrabold text-purple-700">
                      ${c.price}
                    </td>
                    ...
                    <td className="px-4 py-4">
                      <div className="flex flex-wrap gap-1.5">
                        {c.isPublished ? (
                          <span className="text-[10px] font-bold bg-emerald-100 text-emerald-800 px-2 py-0.5 rounded">Published</span>
                        ) : (
                          <span className="text-[10px] font-bold bg-slate-200 text-slate-700 px-2 py-0.5 rounded">Draft</span>
                        )}

                        {c.isApproved ? (
                          <span className="text-[10px] font-bold bg-blue-100 text-blue-800 px-2 py-0.5 rounded">Approved</span>
                        ) : (
                          <span className="text-[10px] font-bold bg-amber-100 text-amber-800 px-2 py-0.5 rounded">Pending</span>
                        )}

                        {c.isFeatured && (
                          <span className="text-[10px] font-bold bg-purple-100 text-purple-800 px-2 py-0.5 rounded flex items-center gap-0.5">
                            <Star className="w-2.5 h-2.5 fill-purple-600" /> Featured
                          </span>
                        )}

                        {c.isArchived && (
                          <span className="text-[10px] font-bold bg-rose-100 text-rose-800 px-2 py-0.5 rounded">Archived</span>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="px-5 py-4 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        ...
                        >
                          {c.isPublished ? <Globe className="w-3.5 h-3.5 text-emerald-600" /> : <Lock className="w-3.5 h-3.5 text-slate-400" />}
                        </Button>
                        ...
                        >
                          <CheckCircle2 className={`w-3.5 h-3.5 ${c.isApproved ? 'text-blue-600' : 'text-slate-400'}`} />
                        </Button>
                        ...
                        >
                          <Star className={`w-3.5 h-3.5 ${c.isFeatured ? 'fill-purple-600 text-purple-600' : 'text-slate-400'}`} />
                        </Button>
                        ...
                        >
                          <Archive className={`w-3.5 h-3.5 ${c.isArchived ? 'text-rose-600' : 'text-slate-400'}`} />
                        </Button>
