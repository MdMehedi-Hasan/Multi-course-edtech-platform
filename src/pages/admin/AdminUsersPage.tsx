  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-100 tracking-tight">Platform User Management</h1>
          <p className="text-xs text-slate-400 mt-1">Manage user roles, inspect account activity, suspend policy violators, and maintain security authorization.</p>
        </div>
      </div>
      ...
      {actionFeedback && (
        <div
          className={`p-4 rounded-xl flex items-center justify-between text-xs font-semibold ${
            actionFeedback.type === 'success' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-rose-500/10 text-rose-400 border border-rose-500/20'
          }`}
        >
          <span>{actionFeedback.message}</span>
          <button onClick={() => setActionFeedback(null)} className="p-1 hover:bg-slate-700 rounded">
            <X className="w-4 h-4" />
          </button>
        </div>
      )}
        ...
        <form onSubmit={handleSearchSubmit} className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 bg-slate-900/60 border border-slate-700/50 rounded-xl text-xs text-slate-100 placeholder:text-slate-500 focus:ring-2 focus:ring-purple-600 outline-none"
            />
          </div>
              ...
              {isLoading ? (
                <tr>
                  <td colSpan={6} className="text-center py-12 text-slate-400">
                    <div className="w-6 h-6 border-2 border-purple-500/40 border-t-transparent rounded-full animate-spin mx-auto mb-2" />
                  ...
                  <tr key={u.id} className="hover:bg-slate-700/40 transition-colors">
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-full bg-slate-700 flex items-center justify-center font-bold text-slate-300 text-xs shrink-0 overflow-hidden">
                          {u.avatarUrl ? (
                        ...
                        <div>
                          <p className="font-bold text-slate-100 flex items-center gap-1.5">
                            {u.name}
                            {u.id === currentUser?.id && (
                              <span className="text-[10px] bg-purple-500/10 text-purple-400 font-bold px-1.5 py-0.5 rounded">You</span>
                            )}
                          </p>
                    ...
                    <td className="px-4 py-4 text-[11px]">
                      ...
                      {u.role === 'INSTRUCTOR' && <div className="text-amber-400 font-medium">{u.courseCount} Authored Courses</div>}
                    </td>
                    ...
                    <td className="px-5 py-4 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        ...
                        >
                          {u.isSuspended ? <UserCheck className="w-3.5 h-3.5 text-emerald-400" /> : <UserX className="w-3.5 h-3.5 text-rose-400" />}
                        </Button>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <Card className="w-full max-w-2xl max-h-[85vh] flex flex-col p-6 shadow-2xl relative">
            <button
              ...
              className="absolute top-4 right-4 p-1 rounded-lg text-slate-400 hover:bg-slate-800"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            {isDetailLoading || !userDetail ? (
              <div className="py-16 text-center">
                <div className="w-8 h-8 border-4 border-purple-500/40 border-t-transparent rounded-full animate-spin mx-auto" />
                <p className="text-xs text-slate-400 mt-2">Loading user dossier...</p>
              </div>
            ) : (
              <div className="space-y-6 overflow-y-auto pr-2">
                <div className="flex items-center gap-4 border-b pb-4">
                  <div className="w-16 h-16 rounded-2xl bg-purple-500/10 text-purple-400 flex items-center justify-center text-xl font-black">
                    {userDetail.name.substring(0, 2).toUpperCase()}
                  </div>
                  <div>
                    <h3 className="text-lg font-extrabold text-slate-100">{userDetail.name}</h3>
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      {userDetail.isSuspended ? (
                        <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-rose-500/10 text-rose-400">Suspended</span>
                      ) : (
                ...
                {/* Enrolled Courses */}
                <div>
                  <h4 className="font-bold text-slate-100 text-xs uppercase tracking-wider mb-2">Enrolled Courses ({userDetail.enrollments?.length || 0})</h4>
                  <div className="space-y-2 max-h-40 overflow-y-auto border-slate-700/50 rounded-xl p-3 bg-slate-800/60 text-xs">
                    {userDetail.enrollments?.length > 0 ? (
                      userDetail.enrollments.map((e: any) => (
                        <div key={e.id} className="flex items-center justify-between p-2 bg-slate-800 rounded-lg border-slate-700/50">
                          <span className="font-semibold text-slate-200">{e.course?.title}</span>
                          <span className="text-[11px] text-purple-400 font-bold">{e.progress}% complete</span>
                        </div>
                      ))
                    ) : (
                ...
                {userDetail.role === 'INSTRUCTOR' && (
                  <div>
                    <h4 className="font-bold text-slate-100 text-xs uppercase tracking-wider mb-2">Authored Courses ({userDetail.authoredCourses?.length || 0})</h4>
                    <div className="space-y-2 max-h-40 overflow-y-auto border-slate-700/50 rounded-xl p-3 bg-slate-800/60 text-xs">
                      {userDetail.authoredCourses?.length > 0 ? (
                        userDetail.authoredCourses.map((c: any) => (
                          <div key={c.id} className="flex items-center justify-between p-2 bg-slate-800 rounded-lg border-slate-700/50">
                            <span className="font-semibold text-slate-200">{c.title}</span>
                            <span className="text-[10px] px-2 py-0.5 rounded font-bold bg-purple-500/10 text-purple-400">
      ...
      {roleChangeUser && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <Card className="w-full max-w-md p-6 shadow-2xl space-y-4">
            <h3 className="font-extrabold text-slate-100 text-base">Change User Role</h3>
            <p className="text-xs text-slate-400">
              Modifying permission level for <span className="font-bold text-slate-100">{roleChangeUser.email}</span>.
            </p>

            <div>
              <label className="block text-xs font-bold text-slate-300 mb-1">Select New Role</label>
              <select
                value={targetRole}
                onChange={(e) => setTargetRole(e.target.value)}
                className="w-full px-3 py-2 bg-slate-900/60 border border-slate-700/50 rounded-xl text-xs font-bold text-slate-100 outline-none"
              >
      ...
      {suspendingUser && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <Card className="w-full max-w-md p-6 shadow-2xl space-y-4">
            ...
            <p className="text-xs text-slate-400">
              Target user: <span className="font-bold text-slate-100">{suspendingUser.email}</span>
            </p>
            ...
            {!suspendingUser.isSuspended && (
              <div>
                ...
                <textarea
                  ...
                  className="w-full p-2.5 bg-slate-900/60 border border-slate-700/50 rounded-xl text-xs text-slate-100 placeholder:text-slate-500 outline-none focus:ring-2 focus:ring-rose-600"
                />
              </div>
            )}
