        ...
        <Card className="p-5 border-l-4 border-l-blue-500 hover:shadow-md transition-shadow">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h3 className="text-2xl font-black text-slate-900 mt-1">{stats?.studentCount || 0}</h3>
        ...
        <Card className="p-5 border-l-4 border-l-amber-500 hover:shadow-md transition-shadow">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h3 className="text-2xl font-black text-slate-900 mt-1">{stats?.instructorCount || 0}</h3>
              <p className="text-[11px] text-amber-600 font-semibold mt-0.5">Faculty status active</p>
            </div>
        ...
        <Card className="p-5 border-l-4 border-l-purple-500 hover:shadow-md transition-shadow">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[11px] text-purple-600 font-semibold mt-0.5">{stats?.publishedCourses || 0} published online</p>
            </div>
        ...
        <Card className="p-5 border-l-4 border-l-emerald-500 hover:shadow-md transition-shadow">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[11px] text-emerald-600 font-semibold mt-0.5">{stats?.totalEnrollments || 0} enrollments</p>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between mb-4 border-b pb-3">
            <div>
              <h3 className="font-extrabold text-slate-900 text-base">Recent Student Enrollments</h3>
              <p className="text-xs text-slate-500">Latest platform course registrations</p>
            </div>
          ...
          <div className="space-y-3">
            {stats?.recentEnrollments && stats.recentEnrollments.length > 0 ? (
              stats.recentEnrollments.map((enr: any) => (
                <div key={enr.id} className="p-3.5 rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-between text-xs">
                  <div>
                    ...
                    <p className="text-purple-700 font-semibold mt-0.5">{enr.courseTitle}</p>
                  </div>
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between mb-4 border-b pb-3">
            <div>
              <h3 className="font-extrabold text-slate-900 text-base">Live Audit Trail Stream</h3>
              <p className="text-xs text-slate-500">Real-time security and administrative events</p>
            </div>
            ...
            {stats?.recentAuditLogs && stats.recentAuditLogs.length > 0 ? (
              stats.recentAuditLogs.map((log: any) => (
                <div key={log.id} className="p-3 rounded-xl bg-slate-900 text-slate-200 text-xs flex items-center justify-between">
                  <div>
                    <span className="font-mono text-[10px] bg-purple-950 text-purple-300 border border-purple-800 px-2 py-0.5 rounded font-bold mr-2">
                      {log.action}
                    </span>
