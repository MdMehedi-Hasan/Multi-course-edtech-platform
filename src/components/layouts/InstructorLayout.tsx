          ...
          <div className="relative" ref={dropdownRef}>
            ...
            >
              ...
              <div className="hidden sm:flex flex-col text-left pr-1">
                ...
                <span className="text-[10px] font-semibold text-amber-400">Author Portal</span>
              </div>
            </button>
            ...
            {userDropdownOpen && (
              <div className="absolute right-0 mt-2 w-52 bg-slate-950 rounded-2xl shadow-2xl border border-slate-800 py-1.5 z-50 animate-in fade-in slide-in-from-top-1 duration-150">
                <div className="px-4 py-2 border-b border-slate-800">
                  ...
                  <span className="inline-block mt-1 px-2 py-0.5 bg-amber-950 text-amber-300 text-[10px] font-extrabold uppercase rounded border border-amber-800/60">
                    Instructor
                  </span>
                </div>
                ...
                <button
                  onClick={() => {
                    setUserDropdownOpen(false);
                    onNavigate('/instructor/profile');
                  }}
                  className="w-full text-left px-4 py-2 text-xs text-slate-300 hover:bg-slate-850 hover:text-amber-400 flex items-center gap-2.5 font-medium transition-colors"
                >
                ...
                <button
                  onClick={() => {
                    setUserDropdownOpen(false);
                    onNavigate('/instructor/settings');
                  }}
                  className="w-full text-left px-4 py-2 text-xs text-slate-300 hover:bg-slate-850 hover:text-amber-400 flex items-center gap-2.5 font-medium transition-colors"
                >
